...
export const updateAlumno = async (req, res) => {
    try {
        const [alumno, errorAlumno] = await updateAlumnoService(req.body, req.params.id);
        if(errorAlumno) return handleErrorClient(res, 404, errorAlumno);
...
export const deleteAlumno = async (req,res) => {
    try {
        ...
        if (errorAlumno) return handleErrorClient(res, 404, "Error elimando al alumno", errorAlumno);
        handleSuccess(res, 200, "Se elimino el registro del alumno exitosamente", alumno);
    } catch (error) {
        handleErrorServer(res, 500, error.message);
    }
};